...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (for {(char, list) <- w.toLowerCase().toList.groupBy(x => x)} yield (char, list.length)).toList.sortBy(_._1)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val wordOccurencesMap = dictionary map (w => (w, wordOccurrences(w)))
    for {
      (o, list) <- wordOccurencesMap.groupBy(x => x match {
        case (w, o) => o
      })
    } yield (o, list.map(_._1))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val matchingCharOccurences = for {
      (cx, lx) <- x
      (cy, ly) <- y
      if (cx == cy && lx - ly > 0)
    } yield (cx, lx - ly)
    val y_chars = y.map(y1 => y1 match {
      case (c, _) => c
    })
    val notMatchingCharOccurences = for {
      (cx, lx) <- x
      if (!y_chars.contains(cx))
    } yield (cx, lx)
    (matchingCharOccurences ::: notMatchingCharOccurences).sortBy(occurence => occurence match {
      case (char, length) => char
    })
  }
